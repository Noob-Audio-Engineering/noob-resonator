...
//!
//! ```text
//! cargo run --release --features plugin --bin benchmark
//! cargo run --release --features plugin --bin benchmark -- --dump series > series.csv
//! ```
//!
...
/// A zero-padded `f64` magnitude spectrum, **unwindowed**.
///
/// A window is right for [`fft_mag`], which analyses a truncated slice of a
/// long tail. This one analyses a whole ring that has decayed into the float
/// floor before the capture ends, so there is no truncation step to taper and
/// a window would only add its own bias to the peak position — and the
/// quantity read off it is a cent at fourteen kilohertz.
fn ring_spectrum(sig: &[f32], n_fft: usize) -> Vec<f64> {
    let mut buf = vec![rustfft::num_complex::Complex::new(0.0f64, 0.0f64); n_fft];
    for (i, s) in sig.iter().enumerate().take(n_fft) {
        buf[i] = rustfft::num_complex::Complex::new(*s as f64, 0.0);
    }
    rustfft::FftPlanner::new()
...
///
/// The window is in cents so that it stays inside the partial spacing at every
/// frequency: at the twentieth harmonic the neighbours are eighty-five cents
/// away, so a forty-cent window cannot report somebody else's partial as this
/// one's.
...
fn guide_section() -> Section {
    let mut s = Section::new(
        ...
        "An open–open column resonates at `n·c/2ℓ` and an open–closed one at `(2n−1)·c/4ℓ`, \
         ...
         Everything a column publishes — the resonance table, the response curve, the length, the \
         round trip — comes out of one closed-form description of the loop, and that description \
         also sizes the delay line, so **the model can agree with itself while disagreeing with \
         the audio**. The first row of each pair strikes the column, transforms what comes out and \
         asks whether the peaks are where the engine said they would be. It is the row that found \
         a shipped column ringing 15.8 cents flat of its own table on a tube and 31.5 on a stopped \
         pipe, from two samples of loop the length solve did not know it had.\n\n\
         **A `Disperse` control was measured here and taken out again.** A first-order allpass in \
         the loop, once per round trip, on the argument that it would give an air column a \
         piano-like stretch. Across its whole travel it moved the inharmonicity of partial 32 by \
         **under a cent** — +0.53 to −0.29 cents from the audio — and not monotonically, because \
         its pole sat next to Nyquist where a bore has nothing left to say. The opposite sign is \
         not small at all, +61.3 cents at partial 64 for a single stage, so the objection is not \
         cost; it is that a first-order allpass has `π` of phase to spend over the whole band and \
         so must saturate, where a stiff string's inharmonicity grows as `n²` without bound, and \
         that a smooth rigid cylinder is very nearly non-dispersive in the first place. Those \
         figures cannot be regenerated by this file because the mechanism is gone; the working is \
         in `src/dsp/guide.rs`.",
    );
    for (name, opening, odd) in [
        ...
        // Does the audio ring where the table says it will? Struck at a
        // thirty-second and heard at a fiftieth, so that the first null of the
        // strike comb and of either pickup comb lands above the twentieth
        // partial and every partial asserted has a peak to find.
        //
        // The pass mark is half a cent rather than a hundredth because the two
        // quantities are not the same quantity: the table publishes where the
        // loop's **pole** is, from the phase condition, and a transform finds
        // where the **peak** of the magnitude is. A resonance sitting on its
        // neighbours' skirts peaks a little off its own pole, by an amount
        // that is a property of its width — 4.2 cents at a half-second decay,
        // 0.24 at four seconds, 0.06 at sixteen.
        let audio_set = guide::Settings {
            f0: 220.0,
            opening,
            radius_mm: 20.0,
            decay: 4.0,
        ...
        let mut audio = guide::Guide::new(SR);
        audio.configure(&audio_set);
        let table: Vec<f64> = audio.resonances().iter().map(|r| r.hz as f64).collect();
        let mag = ring_spectrum(&ring_guide(&audio_set, 1 << 18), 1 << 20);
        let mut off = 0.0f64;
        let mut missing = 0usize;
        for want in table.iter().take(20) {
            let got = peak_near(&mag, SR as f64, *want, 40.0);
            if !got.is_finite() {
                missing += 1;
                continue;
            }
            let e = 1200.0 * (got / want).log2();
            if e.abs() > off.abs() {
                off = e;
            }
        }
        s.meets(
            format!("{name}: the audio against the table the engine published, first 20"),
            "within 0.5 cents, and every one of them found",
            if missing == 0 {
                format!("{off:+.3} cents")
            } else {
                format!("{off:+.3} cents, and {missing} with no peak to find")
            },
            off.abs() < 0.5 && missing == 0,
            "the loop's own algebra against a transform of what it played; \
             `an_air_column_rings_at_the_frequencies_it_publishes` asserts the same thing",
        );
        ...
        s.note(
            ...
            "the third-order Lagrange fractional delay and the loss filter in the loop; a real \
             column disperses too, and the 16-resonance row is a band-limited claim rather than \
             a claim about the whole band. The number is the engine's own table, which the audio \
             row at the top of this pair checks against what the column actually played",
        );
    }
...
fn main() {
    ...
    out.push_str(
        "Generated by `cargo run --release --features plugin --bin benchmark`. Every row names the figure, where it \
