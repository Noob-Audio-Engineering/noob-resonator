...
impl Processor {
    ...
    pub fn configure(&mut self, s: &Settings) {
        ...
        let mut s = *s;
        let overriding = s.midi_voices && s.object_can_voice();
        if overriding {
        ...
        // **The readout gets the voicing only where it actually took the
        // pitch**, which is the same condition the override ran under and
        // not merely `midi_voices`. A surface takes no voices, so its one
        // pitch is its parameter's whatever the keyboard is doing, and
        // saying otherwise would grey a control that is working.
        self.engine.set_held(if overriding {
            self.voicing
        } else {
            Voicing::default()
        });
